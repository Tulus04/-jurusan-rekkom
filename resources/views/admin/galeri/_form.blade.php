...
<div class="row">
    <div class="col-md-8">
        <div class="card mb-3"><div class="card-body">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" id="deskripsi" name="deskripsi" rows="3">{{ old('deskripsi', $galeri->deskripsi ?? '') }}</textarea>
            </div>
            <div class="mb-3">
                <label for="kategori" class="form-label">Kategori</label>
                <select class="form-select" id="kategori" name="kategori">
                    <option value="">Pilih Kategori</option>
                    @foreach(['Kegiatan','Fasilitas','Prestasi','Lainnya'] as $k)
    ...
    <div class="col-md-4">
        <div class="card mb-3"><div class="card-header"><strong>Gambar</strong></div><div class="card-body">
            ...
            <div class="mt-2"><img id="gambar-preview" src="{{ isset($galeri) && $galeri->gambar ? asset('storage/' . $galeri->gambar) : '' }}" class="img-fluid rounded {{ isset($galeri) && $galeri->gambar ? '' : 'd-none' }}" style="max-height:200px;"></div>
        </div></div>
        <div class="card mb-3"><div class="card-header"><strong>Pengaturan</strong></div><div class="card-body">
            <div class="mb-3"><label for="urutan" class="form-label">Urutan</label><input type="number" class="form-control" id="urutan" name="urutan" value="{{ old('urutan', $galeri->urutan ?? 0) }}" min="0"></div>
            <div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $galeri->is_active ?? true) ? 'checked' : '' }}><label class="form-check-label" for="is_active">Aktif</label></div>
        </div></div>
    </div>
</div>
